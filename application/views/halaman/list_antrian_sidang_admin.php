            ...
            <div class="table-responsive">
                <table id="tabel-antrian" class="table table-bordered table-hover table-striped mb-4">
                    ...
                    if(intval($no_dipanggil) > 0) {
                        $disabled='';
                        echo "<td style='vertical-align:top;' align='center'><font size='14px' style='color: red;'><b>".$no_dipanggil_cetak."</b></font></td>";
                    } else {
                        $disabled='disabled';
                    ...
                    <td style='vertical-align:top' align='center'>
                        <div class='btn-group-vertical' role='group' aria-label='Basic example'>
                            <button type='button' class='btn btn-primary panggil_sidang'  data-no_antrian='".encrypt_url($no_dipanggil)."' data-no_antrian_cetak='".$no_dipanggil_cetak."' data-noperk='".encrypt_url($noperk_dipanggil)."' data-noperks='".$noperk_dipanggil."' data-tglsidang='".$tgl_sidang."' data-ruang='".$ruang_nama."' data-ruangan_id=$ruangan_id data-dilewat=0 $disabled>Panggil</button>
                            <button type='button' class='btn btn-info panggil_sidang_asing'  data-no_antrian='".encrypt_url($no_dipanggil)."' data-no_antrian_cetak='".$no_dipanggil_cetak."' data-noperk='".encrypt_url($noperk_dipanggil)."' data-noperks='".$noperk_dipanggil."' data-tglsidang='".$tgl_sidang."' data-ruang='".$ruang_nama."' data-ruangan_id=$ruangan_id data-dilewat=0 $disabled>Panggil Bhs Asing</button>
                            <button type='button' class='btn btn-danger lewati_sidang'   data-no_antrian='".encrypt_url($no_dipanggil)."' data-no_antrian_cetak='".$no_dipanggil_cetak."' data-noperk='".encrypt_url($noperk_dipanggil)."' data-tglsidang='".$tgl_sidang."' data-ruang='".$ruang_nama."'  data-ruangan_id=$ruangan_id data-dilewat=0 $disabled>Skors/Lewati</button>
                        </div>
                    </td>";
                    echo"
                    <td style='vertical-align:top;' align='center'>
                        <div class='btn-group-vertical' role='group' aria-label='Basic example'>
                            <button type='button' class='btn btn-success mulai_sidang'  data-no_antrian='".encrypt_url($no_dipanggil)."' data-noperk='".encrypt_url($noperk_dipanggil)."' data-tglsidang='".$tgl_sidang."' data-ruang='".$ruang_nama."' data-ruangan_id=$ruangan_id data-dilewat=0 $disabled >Mulai</button>
                              <button type='button' class='btn btn-danger selesai_sidang' data-no_antrian='".encrypt_url($no_dipanggil)."' data-noperk='".encrypt_url($noperk_dipanggil)."' data-tglsidang='".$tgl_sidang."' data-ruang='".$ruang_nama."' data-ruangan_id=$ruangan_id data-dilewat=0 $disabled >Selesai</button>
        ...
        <div class="widget-content-area br-4">
            <table id="data-tabel" class="table table-striped">
                ...
                if ($daftar_sidang->num_rows() > 0) {
                    $no=1;
                    //echo"<pre>";
                    //print_r($daftar_sidang->result());exit;
                    foreach ($daftar_sidang->result() as $row) {
                        ...
                        echo "<td style='vertical-align:top'>$row->jurusita<br> <button type='button' class='btn-sm btn-danger relaas' data-noperk='".$row->nomor_perkara."'   data-tgl_sidang='".$row->tgl_sidang."'  data-perkara_id=$row->perkara_id>relaas</button></td>";
                            if($durasi==1) {

                                if(is_null($row->durasi) or $row->durasi=='') {
                                    echo "<td style='text-align:middle; vertical-align:top'><button type='button' class='btn btn-primary btn-rounded waves-effect waves-light durasi' pkrid=$row->perkara_id tglsidang='".$tgl_sidang."' ruang='".$ruang_nama."' id='tb".$no."'>Durasi</button></td>";
                                } else {
                                    echo "<td style='text-align:middle; vertical-align:top'><span class='badge-pill badge-danger' style='font-size:20px'>$row->durasi</span><button type='button' class='btn btn-primary btn-rounded btn-sm waves-effect waves-light edit_durasi pull-right' pkrid=$row->perkara_id tglsidang='".$tgl_sidang."' ruang=$ruang_nama durasi=$row->durasi id='tb".$no."'>Edit</button></td>";
                                }
                            ...
                            if(!is_null($row->ruangan_id)) {
                                echo "<td style='text-align:middle; vertical-align:top'><span class='badge badge-info'>".$row->nama_ruang." </span></td>";
                            } else {
                                echo "<td style='text-align:middle; vertical-align:top'><button type='button' class='btn btn-small btn-danger btn-rounded waves-effect waves-light set_ruang' data-noperk='".$row->nomor_perkara."' data-pkrid=$row->perkara_id data-tgl_sidang='".$this->uri->segment(3)."' >Ruangs</button></td>";
                            }
                            ...
                            if($row->status==1) {
                                echo "<td style='text-align:middle; vertical-align:top'><span class='badge-pill badge-success'>selesai</span></td>";
                            } else if ($row->status==2) {
                                echo "<td style='text-align:middle; vertical-align:top'><span class='badge-pill badge-danger'>dilewati</span></td>";
                            } else if ($row->status==3){
                                echo "<td style='text-align:middle; vertical-align:top'><span class='badge-pill badge-warning'>sidang</span></td>";
                            } else if ($row->status==4) {
                                echo "<td style='text-align:middle; vertical-align:top'><span class='badge-pill badge-secondary'>dipanggil</span></td>";
                            } else {
                                echo "<td style='text-align:middle; vertical-align:top'>BELUM DIPANGGIL</td>";
                            }
...
<script>
    ...
    function list_dilewat(a) {
        window.open("<?php echo base_url(); ?>utama/list_dilewat_admin/"+a, "_self", "toolbar=yes,scrollbars=yes,resizable=yes,top=500,left=500,width=400,height=400");
    }
