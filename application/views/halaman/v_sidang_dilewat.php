...
<div class="row layout-top-spacing" id="cancel-row">
 <div class="col-xl-12 col-lg-12 col-sm-12">
        ...
        <h3>DAFTAR ANTRIAN SIDANG DILEWAT/DISKORS</h3>
            ...
            <a href="<?php echo base_url(); ?>utama/list_sidang/<?php echo $tgl_enc; ?>"><button type='button' class='btn btn-danger float-right'>Kembali Ke Antrian Utama</button></a>
        </div>
    </div>
</div>
<div class="row layout-top-spacing" id="cancel-row">
                ...
                <table id="data-tabel" class="table table-striped">
                    ...
                    if ($daftar_sidang->num_rows() > 0) {
                        ...
                        foreach ($daftar_sidang->result() as $row) {
                            ...
                            echo "<td style='vertical-align:top'>".($this->session->userdata('iduser')==$row->iduser?"<font color=red>".$row->majelis."</font>":$row->majelis)."</td>";
                            ...
                            if  ($this->session->userdata('kewenangan')==1) {
                                ...
                                echo "<td style='vertical-align:top;' ><div class='btn-group-vertical'>
                                        ...
                                        <button type='button' class='btn btn-success btn-sm panggil_saksi' data-pihak='P' data-no_antrian='".encrypt_url($row->no_antrian)."' data-noperk='".$noperk."' $disabled>Pgl Saksi P</button>
                                 <button type='button' class='btn btn-info btn-sm panggil_saksi' data-pihak='T' data-no_antrian='".encrypt_url($row->no_antrian)."'data-noperk='".$noperk."' $disabled>Pgl Saksi T</button> 
                                <button type='button' class='btn btn-success btn-sm  panggil_non_sidang' data-jenis=5 data-ruangan_id=$ruangan_id  data-ruang='".$ruangsidang."' $disabled>Pgl Petugas</button>
                                </div></td>";
                                echo "<td style='vertical-align:top;' ><div class='btn-group-vertical'>
